module ActiveScaffold
  module Constraints
    ...
    def apply_constraints_to_record(record, options = {})
      ...
      #FIXME Se añade que tenga en cuenta el params[:defaults]
      defaults = params[:defaults] || {}
      #FIXME Añadimos el active_scaffold_session_storage[:defaults]. Se debería usar uno solo.
      defaults.merge!(active_scaffold_session_storage[:defaults] || {})

      defaults.merge(active_scaffold_constraints).each do |k, v|
        column = active_scaffold_config.columns[k]
